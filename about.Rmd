...
**Metodología de asimetría y curva de densidad tomado de:**[@caballero1975introduccion]
<br>
<div/>
...
ggplot(top_10, aes(x = reorder(municipios, frecuencia), y = frecuencia, fill = frecuencia)) +
  ...
  geom_text(aes(label = frecuencia), hjust = -0.3, size = 3.5) +
  coord_flip() +
  labs(title = "Top 10 municipios con más \nhurtos y delincuencia",
...
ggplot(tabla_frecuencias_victima, aes(x = reorder(victima, frecuencia), y = frecuencia, fill = frecuencia)) +
  ...
  geom_text(aes(label = frecuencia), hjust = -0.3, size = 3.5) +
  coord_flip() +
  labs(title = "Móvil de la victima en el \nmomento del hurto",
       x = "Móviles", y = "Frecuencia") +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 11), # Ajustar el tamaño del texto de las categorías
        axis.text.x = element_text(size = 11),
...
<br>
<div style="text-align: justify">
Con el presente diagrama de sectores, se visualiza de mejor manera la distribución porcentual de los datos obtenidos con respecto al sexo, mostrando que, en efecto los hombres tienden a presentar más casos de hurto que las mujeres, lo cual identifica a los hombres como el grupo vulnerable y más expuesto a los casos de hurtos y delincuencia.
<div/>
<br>
...
ggplot(tabla_frecuencias_categoria, aes(x = categoria, y = frecuencia)) +
  ...
  geom_text(aes(label = frecuencia), vjust = -0.5, size = 3.5) +
  theme_minimal() +
  labs(title = "Frecuencia por categoría de hurto", x = "Categoría de gravedad", y = "Frecuencia") +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
    axis.text.y = element_text(size = 12),
    axis.title.x = element_text(size = 12,face = "bold"),
